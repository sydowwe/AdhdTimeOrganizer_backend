...
namespace AdhdTimeOrganizer.Tracking.infrastructure.persistence.seeder.dev;
    ...
    ILogger<WebExtensionDataSeeder> logger) : IPerUserDevSeeder, IScopedService
{
    ...
    public async Task SeedForUser(long userId)
    {
        ...
        if (await dbContext.Set<WebExtensionActivityEntry>().IgnoreQueryFilters().AnyAsync(x => x.UserId == userId))
            return;
        ...
        await dbContext.Set<WebExtensionActivityEntry>().AddRangeAsync(records);
