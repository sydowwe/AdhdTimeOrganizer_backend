...
handler **by key**. Owning modules (Reminders, Exports, the existing per-module jobs, …) keep their job *logic* and register a recurring schedule against the `Sydowwe.Framework.Contracts.scheduling` contract; Scheduler
...
- `domain-map.md` — model, invariants, the `Sydowwe.Framework.Contracts` contract, file index
