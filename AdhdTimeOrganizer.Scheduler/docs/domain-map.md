...
## The `Kernel.scheduling` contract (in `MojaDigitalnaFirma.Kernel`)
...
| IScheduledRunReplayer / ScheduledRunReplayer                                                                    | Service       | Module-internal (NOT on the Kernel contract): loads a past run, validates it + its handler, fires the dispatcher with a `TriggerSource = Replay` snapshot data map; writes no run row itself (04b)                                                                                                                                   | `domain/serviceContract/` + `infrastructure/ScheduledRunReplayer.cs`             |
...
| (contract)                                                                                                      | Kernel        | `Kernel.scheduling` — see table above                                                                                                                                                                                                                                                                                                | `MojaDigitalnaFirma.Kernel/scheduling/`                                          |
