...
Owns *when a notification fires on a schedule*. Owning modules register reminders/deadlines as data through a `Sydowwe.Framework.Contracts` contract; this module runs a single recurring scan that finds due occurrences and
...
No module-specific setup beyond the standard EF migrations. The module is complete: the `Sydowwe.Framework.Contracts.reminders`
...
- `summary.md` — start here if you're working in this module (the three-module split + the `Sydowwe.Framework.Contracts` surface).
