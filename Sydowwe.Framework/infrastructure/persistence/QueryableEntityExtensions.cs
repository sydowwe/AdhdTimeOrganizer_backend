...
using Sydowwe.Framework.infrastructure.extension;
...
public static class QueryableEntityExtensions
{
    ...
    public static async Task<List<TResponse>> GetAllMappedAsync<TEntity, TResponse>(this IQueryable<TEntity> query, Expression<Func<TEntity, TResponse>> map)
        ...
        await query.AsNoTracking().Select(map).ToListAsync();
        ...
        where TEntity : class, IEntity
    {
        ...
        {
            return Result<int>.Error(ResultErrorType.UnknownError, "Failed to count entities", e.Message);
        }
    ...
    public static IQueryable<TEntity> SortByMany<TEntity>(this IQueryable<TEntity> query, SortByRequest[] sortByList)
        where TEntity : class
    {
        //WARNING: Can fail with views without id
        if (sortByList.Length == 0)
            query = query.OrderBy(p => EF.Property<TEntity>(p, "Id"));
        else
            foreach (var sortBy in sortByList)
            {
                var key = sortBy.Key.Pascalize();
                var param = Expression.Parameter(typeof(TEntity), "p");
                var prop = Expression.Convert(Expression.PropertyOrField(param, key), typeof(object));
                var orderBy = Expression.Lambda<Func<TEntity, object>>(prop, param);
    ...
    public static IQueryable<TEntity> SortByManyAndPaginate<TEntity>(this IQueryable<TEntity> query, SortByRequest[] sortByList, int itemsPerPage, int page)
        where TEntity : class =>
        query.SortByMany(sortByList).Skip((page - 1) * itemsPerPage).Take(itemsPerPage);
    ...
    private static string ToWildcardSqlPattern(string pattern) => pattern.Replace("%", "\\%").Replace("_", "\\_").Replace("*", "%").Replace("?", "_");
