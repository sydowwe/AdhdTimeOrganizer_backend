...
// EF1001: NpgsqlMigrationsSqlGenerator's only public ctor requires the internal
// INpgsqlSingletonOptions. Subclassing the generator (to append PARTITION BY) is
// impossible without forwarding it, and EF injects it from its own service provider.
// This is the EF-sanctioned pattern for custom SQL generators — suppress narrowly.
...
#pragma warning restore EF1001
        ...
        bool terminate = true)
    {
        ...
        if (partitionsJson is null)
            return;

        var partitions = JsonHelper.Deserialize<List<PartitionDefinition>>(partitionsJson);
