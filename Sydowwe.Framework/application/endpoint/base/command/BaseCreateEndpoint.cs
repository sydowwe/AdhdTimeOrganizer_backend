...
public abstract class BaseCreateEndpoint<TEntity, TRequest>(DbContext dbContext) : Endpoint<TRequest, long>
    where TEntity : class, IEntityWithId
    where TRequest : class, ICreateRequest<TEntity>
{
    /// <summary>
    /// The ambient context, so a subclass hook (e.g. a post-commit <see cref="AfterSave"/> sync) can use it
    /// without re-capturing the constructor parameter. Mirrors <see cref="BaseUpdateEndpoint{TEntity,TRequest}"/>.
    /// </summary>
